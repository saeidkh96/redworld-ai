...
from redworld.domains.autonomy.models import CommunityGroup, Institution, InstitutionKind, SocialNorm, SocietyState
...
class AutonomousSocietyService:
    def bootstrap(self, state: SocietyState, citizens: list[Citizen]) -> None:
        if state.institutions:
            return
        for name, kind in (("Genesis Civic Council", InstitutionKind.COUNCIL), ("Public Forum", InstitutionKind.MEDIA), ("Learning Commons", InstitutionKind.EDUCATION), ("Neighborhood Assembly", InstitutionKind.COMMUNITY)):
            inst = Institution(name, kind)
            state.institutions[str(inst.id)] = inst
        for name in ("mutual_aid", "civic_duty", "fair_exchange", "public_care", "environmental_stewardship"):
            state.norms[name] = SocialNorm(name)
        buckets = [CommunityGroup("North Community"), CommunityGroup("Central Community"), CommunityGroup("South Community")]
    ...
    def daily_update(self, *, tick: int, state: SocietyState, citizens: list[Citizen], profiles: dict[str, CitizenLifeProfile], events: object) -> None:
        if not citizens:
            return
        ...
        state.civic_participation = clamp(0.65 * state.civic_participation + 0.35 * ((belonging + autonomy) / 2))
        state.collective_agency = clamp((state.civic_participation + state.social_cohesion + autonomy) / 3)
        ...
        for institution in state.institutions.values():
            ...
            institution.participation = clamp(0.8 * institution.participation + 0.2 * state.civic_participation)
            institution.legitimacy = clamp((institution.trust + institution.participation + state.social_cohesion) / 3)
        ...
        if state.civic_participation >= 0.45 and tick % 96 == 0:
            ...
            events.append(DomainEvent("CollectiveDecisionMade", tick, {"decision": state.decisions_made, "agency": round(state.collective_agency, 3)}))
